    ...
    <div className="max-w-xl mx-auto space-y-6 pb-6">
      ...
      <section className="bg-brand-card border-3 border-brand-outline p-6 rounded-3xl shadow-pixel text-center space-y-3 relative overflow-hidden">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-brand-text/75 font-semibold font-pixel">
      ...
      {/* 2. TAB SWITCHER */}
      <div className="grid grid-cols-2 p-1.5 bg-brand-border/30 border-3 border-brand-outline rounded-2xl">
        <button
        ...
        {filteredChallenges.length > 0 ? (
          filteredChallenges.map((ch) => {
            ...
            return (
              <div
                key={ch.id}
                className="bg-brand-card border-3 border-brand-outline p-5 rounded-3xl shadow-pixel flex flex-col justify-between gap-4"
              >
                ...
                {ch.completed ? (
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 rounded-2xl border-3 border-brand-outline font-pixel font-bold text-xs shadow-pixel hover:shadow-pixel-hover active:translate-y-0 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                      ch.type === "weekly"
